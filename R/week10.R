...
gss_train_tbl <- gss_tbl_processed[train_idx, ]
gss_test_tbl <- gss_tbl_processed[-train_idx, ]
...
random_forest_grid <- expand.grid(mtry = seq(2, floor(sqrt(ncol(gss_train_tbl))), length.out = 5))
...
formula <- HRS2 ~ .
models <- list(
  OLS = train(formula, data = gss_train_tbl, method = "lm", trControl = train_control),
  ElasticNet = train(formula, data = gss_train_tbl, method = "glmnet", trControl = train_control, tuneGrid = elastic_net_grid),
  RandomForest = train(formula, data = gss_train_tbl, method = "rf", trControl = train_control, tuneGrid = random_forest_grid),
  XGBoost = train(formula, data = gss_train_tbl, method = "xgbTree", trControl = train_control, tuneGrid = xgboost_grid)
)
